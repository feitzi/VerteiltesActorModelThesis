...
Die Speicherung von Ergebnis führt jedoch zu Problemen wenn man die Verteilung der Komponente beachtet. Da jede Instanz seine eigenen Ergebnisse vorbereitet müssen diese auch eigenständig gespeichert werden. Dafür kann entweder pro Instanz eine eigene Persistierung, wie beispielsweise eine Datenbank, vorgehalten werden. Jedoch erhöht dies den Wartungsaufwand der Komponente. Eine weitere möglichkeit ist dafür die \textit{Snapshot} Möglichkeiten von Event Sourcing, welche im Abschnitt \ref{subsec:implementation:eventSouring} beschrieben sind, zu verwenden. Dabei wird für die aktuelle Instanz 
